--[[
    InfinityServerExecutor.server.lua
    By FriendlyBiscuit
    05/01/2022 @ 20:36:28
    
    Description:
        Infinity 6 main server job executor.
--]]
...
--= Object References =--
local shared_jobs           = game.ReplicatedStorage:WaitForChild('InfinityShared'):WaitForChild('jobs')
...
local function format(template: string, ...: any): string
    return '[InfinityServer] ' .. MESSAGES[template]:format(...):gsub('%$REP', string.rep('-', 40))
end
